...
class PINN(tf.keras.Model):
    ...
    def train_op(self, t_ode, t_u, u, lam):
        with tf.GradientTape() as tape:
            ODE = self.ODE(t_ode)
            loss_ode = tf.reduce_mean(ODE ** 2)
            eq1, eq2, eq3, eq4, eq5, eq6, eq7, eq8 = tf.split(ODE, 8, axis=-1)
            # loss_ode =  tf.reduce_mean(eq7 ** 2)
            loss_u = tf.reduce_mean((self.call(t_u) - u) ** 2)
            total_loss = self.eps * loss_ode + loss_u # + lam * tf.reduce_mean(eq5 ** 2)
        grads = tape.gradient(total_loss, self.trainable_variables)
        self.opt.apply_gradients(zip(grads, self.trainable_variables))
        return total_loss, loss_ode, loss_u

    def train(self, t_ode, t_u, u, niter=20000, lam=0):
        ...
        min_loss = 1000
        for i in range(niter):
            loss_value, ode_loss_value, data_loss_value = train_op(t_ode, t_u, u, lam)
            loss += [loss_value.numpy()]
        ...
        return loss, ode_loss, data_loss

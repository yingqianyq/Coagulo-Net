...
class NN(tf.keras.Model):
    ...
    def __init__(self, layers=[1, 20, 20, 1], name="pinn"):
        super().__init__()
        ...
        self.log_h_2 = tf.Variable(0.0, dtype=tf.float32, trainable=True)
    ...
    def ode(self, t):
        ...
        # equations, involving u4, u6, u9, u10, u12, u14, u18, u31, u32, u33
        # step 1: query scaling factors
        # TODO: query scaling factors, s4, s6, s9, s10, s12, s14, s18, s31, s32, s33
        ...
        # eq14 = u14_t - (k_2t*u10*u13 - k_2_plus*2000*u18*u14 + k_2_minus*u16 - h_2*u33*u14) # IIa
        eq14 = u14_t - self.TIM * (k_2t*u10*self.NDM[10]*s10*u13*self.NDM[13]*s13 - k_2_plus*2000*u18*self.NDM[18]*s18*u14*self.NDM[14]*s14 + k_2_minus*u16*self.NDM[16]*s16 - h_2*u33*self.NDM[33]*s33*u14*self.NDM[14]*s14) / self.NDM[14] / s14 # IIa
        # eq16 = u16_t - ((k_2*u28*u15)/(K_2M + u15) + k_2_plus*2000*u18*u14 - k_2_minus*u16) # IIa(m)
        # eq16 = u16_t - self.TIM * ((k_2*u28*self.NDM[28]*s28*u15*self.NDM[15]*s15)/(K_2M + u15*self.NDM[15]*s15) + k_2_plus*2000*u18*self.NDM[18]*s18*u14*self.NDM[14]*s14 - k_2_minus*u16*self.NDM[16]*s16)  / self.NDM[16] / s16 # IIa(m)
        # eq31 = u31_t - (-h_10_TPplus*u10*s10*u31*s31 + h_10_TPminus*u32*s32) / s31  # TFPI
        eq31 = u31_t - self.TIM * (-h_10_TPplus*u10*self.NDM[10]*s10*u31*self.NDM[31]*s31 + h_10_TPminus*u32*self.NDM[32]*s32) / self.NDM[31] / s31 # TFPI
        ...
        # eq33 = u33_t - (-u33*(h_10_AT*u10 + h_9*u6 + h_2*u14 + h_7_AT*u4)) # ATIII
        eq33 = u33_t - self.TIM * (-u33*self.NDM[33]*s33*(h_10_AT*u10*self.NDM[10]*s10 + h_9*u6*self.NDM[6]*s6 + h_2*u14*self.NDM[14]*s14 + h_7_AT*u4*self.NDM[4]*s4)) / self.NDM[33] / s33 # ATIII
        ...
        # eq31 = u31_t - (-h_10_TPplus*u10*u31 + h_10_TPminus*u32) # TFPI
        # eq32 = u32_t - (h_10_TPplus*u10*u31 - h_10_TPminus*u32 - h_7_TP*u4*u32) # xa:TFPI
        return tf.concat([eq10, eq14, eq31, eq32, eq33], axis=-1)
    ...
    def train_op(self, t_f, f, t_u_model, u_model, t_u_exp, u_exp, eps):
        with tf.GradientTape() as tape:
            u_model_pred = self.call(t_u_model)
            u_exp_pred = self.call(t_u_exp)
            # loss_u = tf.reduce_mean((u_pred - u)**2)
            loss_u = tf.reduce_mean((u_model_pred[:, 4:5] - u_model[:, 4:5]) ** 2) + \
                     tf.reduce_mean((u_model_pred[:, 6:7] - u_model[:, 6:7]) ** 2) + \
                     tf.reduce_mean((u_model_pred[:, 9:10] - u_model[:, 9:10]) ** 2) + \
                     tf.reduce_mean((u_model_pred[:, 10:11] - u_model[:, 10:11]) ** 2) + \
                     tf.reduce_mean((u_model_pred[:, 12:13] - u_model[:, 12:13]) ** 2) + \
                     tf.reduce_mean((u_model_pred[:, 13:14] - u_model[:, 13:14]) ** 2) + \
                     tf.reduce_mean((u_model_pred[:, 14:15] - u_model[:, 14:15]) ** 2) + \
                     tf.reduce_mean((u_exp_pred[:, 16:17] - u_exp) ** 2) + \
                     tf.reduce_mean((u_model_pred[:, 18:19] - u_model[:, 18:19]) ** 2) + \
                     tf.reduce_mean((u_model_pred[:, 31:32] - u_model[:, 31:32]) ** 2) + \
                     tf.reduce_mean((u_model_pred[:, 32:33] - u_model[:, 32:33]) ** 2) + \
                     tf.reduce_mean((u_model_pred[:, 33:34] - u_model[:, 33:34]) ** 2)
            u4_loss = tf.reduce_mean((u_model_pred[:, 4:5] - u_model[:, 4:5]) ** 2)
            u9_loss = tf.reduce_mean((u_model_pred[:, 9:10] - u_model[:, 9:10]) ** 2)
            u10_loss = tf.reduce_mean((u_model_pred[:, 10:11] - u_model[:, 10:11]) ** 2)
            u12_loss = tf.reduce_mean((u_model_pred[:, 12:13] - u_model[:, 12:13]) ** 2)
            u16_loss = tf.reduce_mean((u_exp_pred[:, 16:17] - u_exp) ** 2)
            u18_loss = tf.reduce_mean((u_model_pred[:, 18:19] - u_model[:, 18:19]) ** 2)
            u31_loss = tf.reduce_mean((u_model_pred[:, 31:32] - u_model[:, 31:32]) ** 2)
            u32_loss = tf.reduce_mean((u_model_pred[:, 32:33] - u_model[:, 32:33]) ** 2)
            u33_loss = tf.reduce_mean((u_model_pred[:, 33:34] - u_model[:, 33:34]) ** 2)
            f_pred = self.ode(t_f)
            ...
            eq10, eq14, eq31, eq32, eq33 = tf.split(f_pred, 5, axis=-1)
            # loss_f = tf.reduce_mean((f_pred - f)**2)
            loss_f = 1e-5 * tf.reduce_mean(eq10 ** 2) + \
                     1e-5 * tf.reduce_mean(eq14 ** 2) + \
                     1 * tf.reduce_mean(eq31 ** 2) + \
                     1 * tf.reduce_mean(eq32 ** 2) + \
                     1e4 * tf.reduce_mean(eq33 ** 2)
            # loss = loss_u + loss_f
        ...
        return loss, loss_u, loss_f, eq10_loss, eq14_loss, eq31_loss, eq32_loss, eq33_loss, u4_loss, u9_loss, u10_loss, u12_loss, u16_loss, u18_loss, u31_loss, u32_loss, u33_loss

    def train(self, t_f, f, t_u_model, u_model, t_u_exp, u_exp, eps, niter=10000):
        t_f = tf.constant(t_f, tf.float32)
        f = tf.constant(f, tf.float32)
        t_u_model = tf.constant(t_u_model, tf.float32)
        u_model = tf.constant(u_model, tf.float32)
        t_u_exp = tf.constant(t_u_exp, tf.float32)
        u_exp = tf.constant(u_exp, tf.float32)
        ...
        for it in range(niter):
            total_loss, data_loss, ode_loss, eq10_loss, eq14_loss, eq31_loss, eq32_loss, eq33_loss, u4_loss, u9_loss, u10_loss, u12_loss, u16_loss, u18_loss, u31_loss, u32_loss, u33_loss = train_op(t_f, f, t_u_model, u_model, t_u_exp, u_exp, eps)

...
class PINN(tf.keras.Model):
    ...
    def __init__(self, name="pinn", eps=1):
        super().__init__()
    ...
    def ode(self, t):
        ...
        eq10 = u10_t - ((k_7_10*u4*u9)/(k_7_10M + u9) - self.h_10_TPplus*u31*u10 + h_10_TPminus*u32 - h_10_AT*u33*u10 - k_10_plus*2700*u18*u10 + k_10_minus*u12) # Xa
        ...
        eq31 = u31_t - (-self.h_10_TPplus*u10*u31 + h_10_TPminus*u32) # TFPI
        eq32 = u32_t - (self.h_10_TPplus*u10*u31 - h_10_TPminus*u32 - h_7_TP*u4*u32) # xa:TFPI
    ...
    def train_op(self, t_f, f, t_u, u):
        with tf.GradientTape() as tape:
            u_pred = self.call(t_u)
            loss_u = tf.reduce_mean((u_pred[:, 16:17] * 10 - u)**2)
            # loss_u = tf.reduce_mean(((u_pred[:, 0] + u_pred[:, 1]) - u)**2)
            # f_pred = self.ode(t_f)
            ...
            # loss_f = tf.reduce_mean((f_pred - f)**2)
            # loss_f = 1.0 * tf.reduce_mean(eq10 ** 2) + \
        ...
        return loss#, loss_u, loss_f, eq10_loss, eq14_loss, eq31_loss, eq32_loss, eq33_loss
    ...
    def train(self, t_f, f, t_u, u, niter=10000):
        ...
        for it in range(niter):
            # total_loss, data_loss, ode_loss, eq10_loss, eq14_loss, eq31_loss, eq32_loss, eq33_loss = train_op(t_f, f, t_u, u)
            total_loss = train_op(t_f, f, t_u, u)
            ...
            if it % 1000 == 0:
                ...
                print(it, current_loss)#, data_loss.numpy(), ode_loss.numpy(), eq10_loss.numpy(), eq31_loss.numpy(), eq32_loss.numpy())

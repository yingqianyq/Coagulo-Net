...
class PINN(tf.keras.Model):
    ...
    def __init__(self, name="pinn", eps1=1, eps2=1):
        super().__init__()
        ...
        self.eps1 = tf.constant(eps1, tf.float32)
        self.eps2 = tf.constant(eps2, tf.float32)
    ...
    def train_op(self, t_f, f, t_u_model, u_model, t_u_exp, u_exp):
        with tf.GradientTape() as tape:
            u_model_pred = self.call(t_u_model)
            loss_u_model = tf.reduce_mean((u_model_pred[:, :14] - u_model[:, :14])**2) + \
                           tf.reduce_mean((u_model_pred[:, 15] - u_model[:, 15])**2) + \
                           tf.reduce_mean((u_model_pred[:, 17:] - u_model[:, 17:])**2)
            u_exp_pred = self.call(t_u_exp)
            # (u_test[:, 14]* NDM[14] * scale_model[14] + u_test[:, 16] * NDM[16] * scale_model[16]) * 1e-9 = u_exp_1 * scale_exp
            # (u_test[:, 14]* 1400.0 * 0.01 + u_test[:, 16] * 10.0 * 100) * 1e-9 = u_exp_1 * 1e-6
            # (u_test[:, 14]* 14 + u_test[:, 16] * 1000) * 1e-3 = u_exp_1
            # u_test[:, 14]* 0.014 + u_test[:, 16] = u_exp_1
            loss_u_exp = tf.reduce_mean(((u_exp_pred[:, 14] * 0.014 + u_exp_pred[:, 16]) - u_exp)**2)
            f_pred = self.ode(t_f)
            ...
            # loss_f = tf.reduce_mean((f_pred - f)**2)
            # ode loss of eq14 and eq16 are physical info for exp data
            loss_f = 0.0 * tf.reduce_mean(eq10 ** 2) + \
                     1.0 * tf.reduce_mean(eq14 ** 2) + \
                     1.0 * tf.reduce_mean(eq16 ** 2) + \
                     0.0 * tf.reduce_mean(eq31 ** 2) + \
                     0.0 * tf.reduce_mean(eq32 ** 2) + \
                     0.0 * tf.reduce_mean(eq33 ** 2)
            # loss = loss_u + loss_f
            loss = loss_u_model + self.eps1 * loss_u_exp + self.eps2 * loss_f
            eq10_loss = tf.reduce_mean(eq10 ** 2)
            eq14_loss = tf.reduce_mean(eq14 ** 2)
            eq16_loss = tf.reduce_mean(eq16 ** 2)
            eq31_loss = tf.reduce_mean(eq31 ** 2)
            eq32_loss = tf.reduce_mean(eq32 ** 2)
            eq33_loss = tf.reduce_mean(eq33 ** 2)
        grads = tape.gradient(loss, self.trainable_variables)
        self.opt.apply_gradients(zip(grads, self.trainable_variables))
        return loss, loss_u_model, loss_u_exp, loss_f, eq10_loss, eq14_loss, eq16_loss, eq31_loss, eq32_loss, eq33_loss

    def train(self, t_f, f, t_u_model, u_model, t_u_exp, u_exp, niter=10000):
        ...
        for it in range(niter):
            total_loss, model_data_loss, exp_data_loss, ode_loss, eq10_loss, eq14_loss, eq16_loss, eq31_loss, eq32_loss, eq33_loss \
                = train_op(t_f, f, t_u_model, u_model, t_u_exp, u_exp)
            ...
            if it % 1000 == 0:
                ...
                print(it, current_loss, model_data_loss.numpy(), exp_data_loss.numpy(), ode_loss.numpy(), \
                      eq14_loss.numpy(), eq16_loss.numpy())

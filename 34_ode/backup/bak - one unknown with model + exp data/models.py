...
class PINN(tf.keras.Model):
    ...
    def __init__(self, name="pinn", eps1=1, eps2=1, eps3=1):
        super().__init__()
        ...
        self.eps3 = tf.constant(eps3, tf.float32)
    ...
    def train_op(self, t_f, f, t_u_model, u_model, t_u_exp, u_exp):
        with tf.GradientTape() as tape:
            ...
            loss_u_model = tf.reduce_mean((u_model_pred[:, :16] - u_model[:, :16])**2) + \
                           tf.reduce_mean((u_model_pred[:, 17:] - u_model[:, 17:])**2)
            u_exp_pred = self.call(t_u_exp)
            # u_test[:, 16] * NDM[16] * scale_model[16] * 1e-9 = u_exp_1 * scale_exp
            # u_test[:, 16] * 10.0 * 100 * 1e-9 = u_exp_1 * 1e-7
            # u_test[:, 16] * 1000 * 1e-2 = u_exp_1
            # u_test[:, 16] * 10 = u_exp_1
            loss_u_exp = tf.reduce_mean((u_exp_pred[:, 16:17] * 10 - u_exp)**2)
            f_pred = self.ode(t_f)
            ...
            loss_f = 10.0 * tf.reduce_mean(eq10 ** 2) + \
                     0.0 * tf.reduce_mean(eq14 ** 2) + \
                     0.0 * tf.reduce_mean(eq16 ** 2) + \
                     1.0 * tf.reduce_mean(eq31 ** 2) + \
                     1.0 * tf.reduce_mean(eq32 ** 2) + \
                     0.0 * tf.reduce_mean(eq33 ** 2)
            # loss = loss_u + loss_f
            loss = self.eps1 * loss_u_model + self.eps2 * loss_u_exp + self.eps3 * loss_f

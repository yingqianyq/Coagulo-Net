...
class PINN(tf.keras.Model):
    ...
    def train_op(self, t_f, f, t_u, u, eps):
        with tf.GradientTape() as tape:
            u_pred = self.call(t_u)
            # loss_u = tf.reduce_mean((u_pred - u)**2)
            loss_u = tf.reduce_mean((u_pred[:, 4:5] - u[:, 4:5]) ** 2) + \
                     tf.reduce_mean((u_pred[:, 6:7] - u[:, 6:7]) ** 2) + \
                     tf.reduce_mean((u_pred[:, 9:10] - u[:, 9:10]) ** 2) + \
                     tf.reduce_mean((u_pred[:, 10:11] - u[:, 10:11]) ** 2) + \
                     tf.reduce_mean((u_pred[:, 12:13] - u[:, 12:13]) ** 2) + \
                     tf.reduce_mean((u_pred[:, 13:14] - u[:, 13:14]) ** 2) + \
                     tf.reduce_mean((u_pred[:, 14:15] - u[:, 14:15]) ** 2) + \
                     tf.reduce_mean((u_pred[:, 16:17] - u[:, 16:17]) ** 2) + \
                     tf.reduce_mean((u_pred[:, 18:19] - u[:, 18:19]) ** 2) + \
                     tf.reduce_mean((u_pred[:, 31:32] - u[:, 31:32]) ** 2) + \
                     tf.reduce_mean((u_pred[:, 32:33] - u[:, 32:33]) ** 2) + \
                     tf.reduce_mean((u_pred[:, 33:34] - u[:, 33:34]) ** 2)
        ...
        return loss, loss_u, loss_f, eq10_loss, eq14_loss, eq31_loss, eq32_loss, eq33_loss, u4_loss, u9_loss, u10_loss, u12_loss, u18_loss, u31_loss, u32_loss, u33_loss
    ...
    def train(self, t_f, f, t_u, u, eps, niter=10000):
        ...
        for it in range(niter):
            total_loss, data_loss, ode_loss, eq10_loss, eq14_loss, eq31_loss, eq32_loss, eq33_loss, u4_loss, u9_loss, u10_loss, u12_loss, u18_loss, u31_loss, u32_loss, u33_loss = train_op(t_f, f, t_u, u, eps)
